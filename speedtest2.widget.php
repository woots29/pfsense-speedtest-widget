...
$selected_if = $_POST['real_if'] ?? null;
$output = null;
$error = null;
if ($selected_if) {
    // find which gateway(s) use this interface
    $matches = [];
    foreach ($gateways as $gwname => $gw) {
        $logical_if = $gw['interface'];
        $real_if = get_real_interface($logical_if);
        if ($real_if === $selected_if) {
            $matches[] = [
                'Gateway Name'      => $gw['name'],
                'Logical Interface' => $logical_if,
                'Real Interface'    => $real_if,
                'Gateway IP'        => $gw['gateway'],
                'Monitor IP'        => $gw['monitor'],
            ];
        }
    }

    if (empty($matches)) {
        $error = "No gateway found using interface: {$selected_if}";
    } else {
        $output = $matches;
    }
}
...
//added
	$intsel = $_REQUEST['interfacenam'];
		
	if ($_REQUEST['serverid']==0){
		//AUTOSELECT
		$results = shell_exec("speedtest -f json --selection-details --accept-license --accept-gdpr --interface=".$intsel );
	} else {
		//MANUAL SERVER SELECTION
		$serverlist = shell_exec("speedtest -f json --servers --accept-license --accept-gdpr");
		$results = shell_exec("speedtest -f json --server-id=" . $_REQUEST['serverid'] . " --selection-details --accept-license --accept-gdpr --interface=".$intsel );
...
<table class="table">
	<tr >
		<td colspan="3" id="test-m" >Select Interface first, then click the spin icon above<td>
	</tr>
	<tr>
		<td>Interface</td>
		<td colspan="2">
			<select name="speedtest-interface" id="speedtest-interface" style="width: 100%">
				<?php
					$seen = [];
					foreach ($gateways as $gw) {
						$real_if = get_real_interface($gw['interface']);
						if (!$real_if || isset($seen[$real_if])) {
							continue; // skip duplicates or invalid
						}
						$seen[$real_if] = true;
						?>
						<option value="<?=htmlspecialchars($real_if)?>" <?=($real_if == $selected_if) ? "selected" : ""?>>
							<?=htmlspecialchars($real_if . " (" . $gw['friendlyifdescr'] . " - " . $gw['gateway'] . ")")?>
						</option>
						<?php
					}
            ?>
...
function update_speedtest() {
	$('#test-m').html("<div class='text-center'><h4>SPEEDTEST IS NOW RUNNING...</h4> DON'T MODIFY SETTINGS.</div>");
    ...
    $.ajax({
        type: 'POST',
        url: "/widgets/widgets/speedtest2.widget.php",
        dataType: 'json',
        data: {
            serverid: $( "#speedtest-host option:selected" ).val(),
      			interfacenam: $( "#speedtest-interface option:selected" ).val()
        },
        success: function(data) {
            update_result(data);
	      		$('#test-m').html("Finished. Select Interface first, then click the spin icon above");
        },
        error: function() {
            update_result(null);
		      	$('#test-m').html("<h4>Error occured.</h4>Try Again Later.");
        },
        complete: function() {
